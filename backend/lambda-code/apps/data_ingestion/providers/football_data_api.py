...
class FootballDataAPIProvider:
    ...
    def get_injuries(
        ...
        """
        Get current injuries/suspensions.

        Args:
            league_code: Our league code (requires season too)
            team_id: API-Football team ID filter
            season: Season year (e.g., 2025); defaults to current
            fixture_id: Filter by a specific fixture instead of league/team

        Returns:
            List of injury records (player/team/reason), or None on error
        """
        params = {}
        ...
        if fixture_id:
            ...
            if league_code:
                if league_code not in self.LEAGUES:
                    return None
                params['league'] = self.LEAGUES[league_code]['id']
            if team_id:
                params['team'] = team_id
            if not season:
                today = date.today()
        ...
        if not params:
            logger.error("get_injuries requires fixture_id, or team_id/league_code")
            return None
    ...
    def sync_injuries_to_database(
        self,
        league_code: str,
        season: Optional[int] = None,
    ) -> int:
        """
        Fetch current injuries for a league and store them as TeamInjury
        rows, matched against existing Team records by normalized name.
        ...
        Returns:
            Number of injury records saved
        """
        from datetime import date as date_cls
        from apps.teams.models import Team, TeamInjury

        injuries = self.get_injuries(league_code=league_code, season=season)
        if not injuries:
            return 0
        ...
        for item in injuries:
            try:
                ...
                team = Team.objects.filter(
                    ...
                    Q(name__iexact=team_name) | Q(name__iexact=normalized)
                ).first()
                if not team:
                    continue

                TeamInjury.objects.update_or_create(
                    team=team,
                ...
                logger.error(f"Error saving injury record: {e}")
                continue

        logger.info(f"Saved {saved} injury records for {league_code}")
        return saved
