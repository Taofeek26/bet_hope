...
class Prediction(TimeStampedModel):
    ...
    # Total goals from the separate goals_model (not derivable from the
    # home/away score fields above, which the pipeline doesn't predict a
    # split for)
    predicted_total_goals = models.DecimalField(
        max_digits=4,
        decimal_places=2,
        null=True,
        blank=True
    )
    # Output of the over25_model (probability of >2.5 total goals)
    over_25_probability = models.DecimalField(
        max_digits=6,
        decimal_places=5,
        null=True,
        blank=True,
