...
class TeamNewsSignal(TimeStampedModel):
    """
    LLM-derived signal extracted from recent news Documents tagged to a
    team, feeding INTO the ML pipeline as a feature (see
    apps/ml_pipeline/features/match_features.py::_get_ai_signal_features) —
    unlike AIRecommendation above, which reads an already-generated
    Prediction and only produces post-hoc explanatory text.
    """
    ...
    as_of_date = models.DateField(help_text="Date this signal was generated")

    # 0.0 (no mentions) - 1.0 (strong/multiple injury or suspension mentions)
    injury_mention_score = models.FloatField(default=0.0)
    # -1.0 (very negative/disruption) - 1.0 (very positive/morale-boosting)
    sentiment_score = models.FloatField(default=0.0)

    source_count = models.PositiveIntegerField(
        default=0, help_text="Number of news articles this signal was derived from"
    )
    provider = models.CharField(max_length=20, blank=True, help_text="LLM provider used")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['team', 'as_of_date'], name='unique_team_signal_per_day'),
        ]

    def __str__(self):
        return f"News signal for {self.team.name} ({self.as_of_date})"
