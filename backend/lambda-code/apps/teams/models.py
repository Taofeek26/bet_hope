...
class TeamInjury(SyncedModel):
    """
    A player unavailability record (injury/suspension) for a team, pulled
    from API-Football's /injuries endpoint. Kept as a simple time-stamped
    snapshot rather than tracking full player history — feature extraction
    only needs "how many key absences did this team have as of match date".
    """
    team = models.ForeignKey(Team, on_delete=models.CASCADE, related_name='injuries')
    player_name = models.CharField(max_length=200)
    injury_type = models.CharField(max_length=100, blank=True, help_text="e.g. 'Missing Fixture'")
    reason = models.CharField(max_length=200, blank=True)
    as_of_date = models.DateField(help_text="Date this record was reported/captured")
    ...
    def __str__(self):
        return f"{self.player_name} ({self.team.name}, {self.as_of_date})"
