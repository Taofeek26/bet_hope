    ...
    <Card>
      <CardHeader className="flex items-center gap-2 sm:gap-3">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-text-secondary">{stats.period}</p>
        </div>
      </CardHeader>
      ...
      <CardBody className="space-y-4 sm:space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs sm:text-sm text-text-secondary">Overall Accuracy</span>
        ...
        {stats.by_confidence && (
          <div>
            <h4 className="text-xs sm:text-sm font-medium text-text-secondary mb-2 sm:mb-3">
              By Confidence Level
            </h4>
              ...
              {Object.entries(stats.by_confidence).map(([level, data]: [string, any]) => (
                <div key={level} className="flex items-center justify-between text-xs sm:text-sm">
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <span className="text-text-muted">{data.total} picks</span>
        ...
        {modelInfo && (
          <div className="pt-3 sm:pt-4 border-t border-border/50">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
              ...
              <span className="text-xs sm:text-sm font-medium text-text-secondary">Active Model</span>
            </div>
...
function StatCard({ icon: Icon, label, value, color }: StatCardProps) {
  return (
    <div className="p-2 sm:p-3 rounded-lg bg-surface/50">
      <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
        <Icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${color}`} />
        <span className="text-[10px] sm:text-xs text-text-secondary">{label}</span>
      </div>
